...
void ExecPort::configure(ConfigurationView* config) {
	...
	if (!changeTypeStr.empty())
        this->openhat->throwSettingException(this->ID() + ": Illegal value for 'ChangeType', expected: 'ChangedToHigh', 'ChangedToLow', or 'AnyChange': " + changeTypeStr);
	...
	if (this->waitTimeMs < 0)
		this->openhat->throwSettingException(this->ID() + ": Please specify a positive value for WaitTime: ", this->to_string(this->waitTimeMs));

	this->resetTimeMs = config->getInt64("ResetTime", this->resetTimeMs);
	if (this->resetTimeMs < 0)
		this->openhat->throwSettingException(this->ID() + ": Please specify a positive value for ResetTime: ", this->to_string(this->resetTimeMs));
